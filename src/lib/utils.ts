...
const turndownService = new TurndownService({
  ...
  .addRule("a", {
    ...
    replacement: function (content, node: Node) {
      ...
      if (href.startsWith("/"))
        newHref = `https://${process.env.CANVAS_BASE_URL}${newHref}`;

      return href === content || href === newHref
        ? `*${content}*`
        : `*[${content}](${newHref})*`;
    },
  });
...
export function truncateMarkdown(
  ...
  const truncateAt =
    markdown.length > descriptionLimit
  ...
  if (markdown.length > descriptionLimit) {
    ...
    const willSliceAsterisk = (string, index) =>
  ...
  if (boldOperatorCount % 2 !== 0) {
    const trail =
      markdown.match(
        new RegExp(`(${escapeRegExp(TRUNCATION_WARNING)})`),
      )?.[1] ?? "";
    ...
    markdown = new RegExp(`\\*\\*.{0,2}${escapeRegExp(trail)}$`).test(markdown)
      ...
      */
        markdown.replace(
          new RegExp(`(\\*\\*.{0,2})${escapeRegExp(trail)}$`),
          (match) => {
            return " ".repeat(match.length) + trail;
          },
        )
      : markdown.replace(
          new RegExp(`.{2}${escapeRegExp(trail)}$`),
          "**" + trail,
        );
  }
