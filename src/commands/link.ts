...
const LINK_SUCCESSFUL =
  "Cool. The service has been linked to Easel successfully.";
...
export class LinkCommand extends Subcommand {
  ...
  public async chatInputCalendar(
    ...
    if (!validateCalendarUrl(url)) {
      await interaction.reply({
        content: strError(
          "The URL you provided is not a valid Canvas calendar URL. Run `/link calendar` without a URL to get instructions on how to get your Canvas calendar URL",
        ),
        ephemeral: true,
      });
    ...
    try {
      ...
      await interaction.editReply({
        content: strSuccess(
          `${LINK_SUCCESSFUL} You can now use the \`/due\` command to check your upcoming deadlines.`,
        ),
      });
    } catch (error) {
  ...
  public async chatInputPanopto(
    interaction: Subcommand.ChatInputCommandInteraction,
  ): Promise<void> {
    await interaction.reply({
      content: strInfo(
        "This subcommand is planned and has not been developed yet 🚧",
      ),
      ephemeral: true,
    });
  }

  public async canvasCalendarInstructions(
    interaction: Subcommand.ChatInputCommandInteraction,
  ): Promise<void> {
    const instructions = [
      "**Disclaimer:**",
