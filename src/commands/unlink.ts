...
@ApplyOptions<Subcommand.Options>({
  name: "unlink",
  description:
    "Unlink various services the University of Auckland uses to Easel",
  subcommands: [
    {
      name: "calendar",
  ...
  async chatInputCalendar(
    ...
    if (user == null || user.canvasCalendar == null) {
      await interaction.reply({
        content: strWarn(
          "You haven't linked your Canvas calendar to Easel yet",
        ),
        ephemeral: true,
      });
    ...
    try {
      ...
      if (user == null) {
        await interaction.editReply({
          content: strWarn(
            "At this time, Easel does not store any data about you.",
          ),
        });
      ...
      await (
        interaction.replied ? interaction.followUp : interaction.editReply
      )({
